...
class AHK extends EventEmitter {
    ...
    keyTap(keys, duration = 0) {
        let keysToPress;

        if (Array.isArray(keys)) {
            keysToPress = keys.map(key => `{${key}${duration ? " down" : ""}}`).join("");
        } else {
            keysToPress = `{${keys}${duration ? " down" : ""}}`;
        }

        this.inputAhk.stdin.write(`${keysToPress}\r\n`);

        if (duration) {
            if (Array.isArray(keys)) {
                keysToPress = keys.map(key => `{${key}${duration ? " up" : ""}}`).join("");
            } else {
                keysToPress = `{${keys}${duration ? " up" : ""}}`;
            }
            setTimeout(() => {
    ...
    keyRepeat(keys, duration, interval, trigger = 0, lastCast = {skill: 0}) {
